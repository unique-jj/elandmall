...
$(function(){
	...
	$('.sch_key li, .sch_word li').find('a').each(function() {
		$(this).focus(function() {
...
function headerControl(){
	if($("div").hasClass("head")){
		...
		var _contentsTopPadding3 = _hdrTopHei + _schKwdHei + _hdrBnrHei // 서브컨텐츠 상단 간격 ( 상단헤더 + 검색 키워드 바 높이 + 배너높이)
		var _hdrBnrTop = _hdrTopHei + _schKwdHei;
		...
		$(window).scroll(function(){		/*상세에서 스크롤 움직일 시, 카테고리 네비도 같이 이동 - 16.08.14 이태영*/	
			...
			if (st > startScroll && ch > vh ){ // 화면 내용이 스크린의 1.5배가 넘지 않을 경우, head_up 되지않도록 함.  2018.12.17 정의찬
				hd_up()
...
$(function(){
	if($('body').hasClass('sch_part') && $("div").hasClass("sch_sort")){
		var offsetSort = $('.sch_sort').offset().top; //최초 좌표
		//var barHeight = $('.sch_kwd').outerHeight(); // 상단 추천 키워드 높이
		var contPadding; // .contents - padding
		var hdHeight; //해더 높이 + top
		var sortHeight = $('.sch_sort').outerHeight(); // sort 바 높이
		$(window).scroll(function(){
			hdHeight = $('header').outerHeight() + parseInt($('header').css('top'));
			contPadding = parseInt($('.contents').css('padding-top'));
			var winST = $(this).scrollTop()+hdHeight/*+barHeight*/+contPadding+sortHeight; //해더 /* + 추천검색어*/ + content_padding + sch_sort_높이
